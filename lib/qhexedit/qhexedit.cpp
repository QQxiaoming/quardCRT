    ...
    , _undoStack(new UndoStack(_chunks, this))
{
    ...
    setHexAreaColor(this->palette().alternateBase().color());
    setHexFontColor(QPalette::WindowText);
...
void QHexEdit::insert(char ch)
{
    insert(_undoStack->currentSize(), ch);
}
...
void QHexEdit::insert(const QByteArray &ba)
{
    insert(_undoStack->currentSize(), ba);
}
...
QByteArray QHexEdit::getSelection()
{
...
void QHexEdit::paintEvent(QPaintEvent *event)
{
    ...
    if (event->rect() != _cursorRect)
    {
        ...
        if (_addressArea) {
            painter.fillRect(QRect(-pxOfsX, event->rect().top(), _pxPosHexX - _pxGapAdrHex/2, height()), viewport()->palette().color(QPalette::Base));
            int linePos = _pxPosHexX - (_pxGapAdrHex / 2);
        ...
        if (_addressArea)
        {
            ...
            for (int row=0, pxPosY = _pxCharHeight; row <= (_dataShown.size()/_bytesPerLine); row++, pxPosY +=_pxCharHeight)
            {
                ...
                painter.setPen(QPen(viewport()->palette().color(QPalette::Text)));
